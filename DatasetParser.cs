...
public class DatasetParser : MonoBehaviour {
    ...
    SerialPort sp = new SerialPort("/dev/tty.usbmodem14201", 115200);     // Change Port
///
    //public SerialPort motor = new SerialPort("/dev/tty.usbmodem14101", 115200);
/// 
...
public bool a = true;
    void UpdateCube()
    {
        ...
        if(a){
            a=!a;
            thread = new Thread(() => StringToArduino("100000000F"));
            //Debug.Log(1);
        }else{
            a=!a;
            thread = new Thread(() => StringToArduino("050000000F"));
            //Debug.Log(0);
        }
...
/*
    public void StringToArduino(System.String toMotor) {
        ...
        if (!motor.IsOpen) { motor.Open(); }

        motor.WriteLine(toMotor);
        motor.BaseStream.Flush();
...
public IEnumerator AsynchronousReadFromArduino(Action<string> callback, Action fail = null, float timeout = float.PositiveInfinity) {
    ...
    void ReadFromConector()
    {
       ...
       /* 
        ...
        dt = float.Parse(stateInfo[0])/1000.0f;
        ...
        //id = int.Parse(stateInfo[10]);
//OI SO ONNLY USE THIS HWNE TWO IMu CONNECTED
        //if(id == 1){
        //    ReadFromConector();
        //}
